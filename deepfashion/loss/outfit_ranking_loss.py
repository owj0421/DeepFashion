...
def outfit_ranking_loss(
        ...
        outfit_wise_pos_matrix = safe_divide(torch.sum(unstack_tensors(outfit_outs.mask, pos_matrix), dim=1), torch.sum(~outfit_outs.mask, dim=-1, keepdim=True) - 1)
        dist_pos, pos_indices = torch.max(outfit_wise_pos_matrix, dim=-1, keepdim=True)
        
        # Compute Negative distances
        neg_matrix = dist_matrix * get_neg_mask(dist_matrix, outfit_outs, pos_indices=pos_indices)
        outfit_wise_neg_matrix = safe_divide(torch.sum(unstack_tensors(outfit_outs.mask, neg_matrix), dim=1), torch.sum(~outfit_outs.mask, dim=-1, keepdim=True) - 1)
        if method == 'batch_all':
            dist_neg = safe_divide(torch.sum(outfit_wise_neg_matrix, dim=-1), torch.count_nonzero(outfit_wise_neg_matrix, dim=-1))
        elif method == 'batch_hard':
            outfit_wise_neg_matrix[outfit_wise_neg_matrix < dist_pos.expand_as(outfit_wise_neg_matrix)] = 0.
